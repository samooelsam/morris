...
$currentCat      = get_category($category_id[0]->term_id);

if($currentCat->term_id){
        $queryArgs = array(
            'post_type' => array( 'post' ),
            'paged' => $paged,
            'cat' => $currentCat->term_id,
            'order' => 'DESC', 
        );?>
			<div class="techvertu-news-wrapper clearfix archive-news">
				<div class="centerize clearfix">
					<div class="techvertu-news-container clearfix swiper-container">
                        <?php 
                        if(is_category('archive')){
                            ?>
                            <div class="alert-bt alert-primary" role="alert">
                                Please note that all information and imagery provided in this blog is accurate as of the time of posting, and readers are encouraged to verify details independently for the most current updates
                            </div>
                        <?php }
                        ?>
...
<?php 
    ...
    get_template_part('includes/no-results-found', 'widget');
}
get_footer();?>